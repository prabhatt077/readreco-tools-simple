...
function calcGST() {
  ...
  if (mode === 'gst-add') {
    ...
    final = base; // "final" here shown as base price after removing GST
  }
...
function calcAge() {
  ...
  if (next < today || (next.getDate() !== dob.getDate() && dob.getMonth() === 1 && dob.getDate() === 29)) {
    // handle Feb 29 fallback and already-passed date
    next = new Date(today.getFullYear() + 1, dob.getMonth(), dob.getDate());
  }
  if (next.getMonth() !== dob.getMonth()) {
    // Feb 29 in a non-leap year rolled to March 1; keep as computed
  }
...
function estimateTaxIndiaNewRegime(annual) {
  // Simplified illustrative slabs (India New Regime style), NOT filing-accurate
  const slabs = [
    { upto: 300000, rate: 0 },
